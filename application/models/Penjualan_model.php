...
class Penjualan_model extends CI_Model {
	...
	public function get_detail($id){
		$sql = "SELECT *, sales_transaction.created_at AS created_at, sales_transaction.updated_at AS updated_at, sales_transaction.deleted_at AS deleted_at 
				...
				JOIN sales_data ON sales_transaction.id_stransaction = sales_data.id_stransaction
				...
				WHERE sales_transaction.id_stransaction = '".$id."' AND sales_transaction.deleted_at = '0000-00-00 00:00:00'";
	...
	public function get_filter_csv($filter = ''){
		$this->db->where('sales_transaction.deleted_at', '0000-00-00 00:00:00');
		$this->db->select('sales_transaction.id_stransaction AS "Kode Transaksi", customer.id_customer AS "Kode Pelanggan", customer.customer_name AS "Nama Pelanggan", customer.customer_phone AS "Telephon", customer.customer_address AS "Alamat Pelanggan", sales_transaction.total_item AS "Banyak Barang", sales_transaction.total_price AS "Harga Total", sales_transaction.is_cash AS "Metode Pembayaran", sales_transaction.pay_deadline_date AS "Tanggal Batas Pembayaran", sales_transaction.created_at AS "Tanggal Transaksi"');
		$this->db->join('sales_data', 'sales_transaction.id_stransaction = sales_data.id_stransaction');
	...
	public function get_filter_array($filter = ''){
		$this->db->select('sales_transaction.id_stransaction AS "Kode Transaksi", customer.id_customer AS "Kode Pelanggan", customer.customer_name AS "Nama Pelanggan", customer.customer_phone AS "Telephon", customer.customer_address AS "Alamat Pelanggan", sales_transaction.total_item AS "Banyak Barang", sales_transaction.total_price AS "Harga Total", sales_transaction.pay_deadline_date AS "Tanggal Batas Pembayaran", sales_transaction.created_at AS "Tanggal Transaksi"');
		$this->db->where('sales_transaction.deleted_at', '0000-00-00 00:00:00');
		$filter['is_cash'] = 0;
		$this->db->join('customer', 'customer.id_customer = sales_transaction.id_customer', 'left');
		$this->db->where($filter);
